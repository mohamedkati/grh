...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (strlen($telephone) != 10 || ($result != "07" && $result != "06")) {
        $data .= "<div class='alert alert-danger'>le numèro de téléphone invalide.</div>";
    }
    ...
    if (empty($data)) {
        $query = $conn->prepare("SELECT email FROM employe WHERE email = :email");
    ...
    if (empty($data)) {
        ...
        $query = $conn->prepare("INSERT INTO employe (nom, prenom, date_naissance,email , telephone, adresse,date_debut_contrat , date_fin_contrat ,type_employe,mot_de_passe ,id_contrat , id_poste)
         VALUES (:nom, :prenom,  :date_naissance, :email, :telephone, :adresse , :date_debut_contrat , :date_fin_contrat,:type_employe ,:mot_de_passe , :id_contrat, :id_poste )");
        $query->execute([
            'nom' => $nom,
            'prenom' => $prenom,
                    ...
                    <form action="" method="post">
                        <div class="card">
                            <div class="card-header">
                                <div class="card-title">Create Employé</div>
                            </div>
